function Invoke-CodeDiagnostics {
    ...
    $uiText=Read-Source $ui; $docText=Read-Source $doc
    $storeText=Read-Source $store; $readerText=Read-Source $reader
    $loadText=Read-Source $loads; $thirdText=Read-Source $third
    ...
    if($uiText -match '50L?\s*\*\s*1024L?\s*\*\s*1024L?'){
        Add-Finding 'M02-P03-OBS-054' 'MEDIA' 'Archivos' 'Límite web fijo de 50 MiB' 'El componente usa un máximo global de 50 MiB.' 'El límite no se informa ni diferencia por proceso.' 'Mostrar el límite y aplicar reglas por tipo documental.'
    }
    ...
    if(($docText -match 'Path\.GetExtension') -or ($docText -match 'Extension')){
        Add-Finding 'M02-P03-OBS-056' 'ALTA' 'Documentos' 'Tipo de archivo guiado por extensión' 'La admisión se apoya en extensión o catálogo.' 'Una extensión no demuestra el contenido real.' 'Validar MIME, firma binaria y contenido.'
    }
    if($storeText -match 'CalcularHash'){
        Add-Finding 'M02-P03-OBS-057' 'POSITIVA' 'Integridad' 'Huella digital implementada' 'El almacenamiento calcula hash.' 'Debe verificarse algoritmo, persistencia y revalidación.' 'Mantener SHA-256 y comprobarlo en descarga y versionado.'
    }
    ...
    if(($uiText -match 'ModuloOrigen') -and ($uiText -match 'EntidadOrigen')){
        Add-Finding 'M02-P03-OBS-059' 'ALTA' 'Integración' 'Origen transversal editable' 'Los campos de origen están expuestos en el formulario.' 'Se permiten referencias arbitrarias y pérdida de trazabilidad.' 'Resolver el origen desde registros reales y bloquear edición libre.'
    }
    if(($uiText -match 'IdCategoriaDocumental') -and ($uiText -match 'IdCarpetaDocumental')){
        Add-Finding 'M02-P03-OBS-060' 'ALTA' 'Clasificación' 'Categoría y carpeta son campos independientes' 'La interfaz administra ambos identificadores por separado.' 'Permite combinaciones incoherentes confirmadas visualmente.' 'Mapear categoría y carpetas, y validar compatibilidad.'
    }
    if($uiText -match '_templateId\s*=\s*_templates\.FirstOrDefault'){
        Add-Finding 'M02-P03-OBS-061' 'MEDIA' 'Cargas' 'Plantilla predeterminada automática' 'Se selecciona la primera plantilla al cargar.' 'Aumenta el riesgo de leer un archivo con plantilla equivocada.' 'Iniciar sin selección y exigir una decisión explícita.'
    }
    if(($readerText -match 'XDocument\.Load') -and ($readerText -match 'ZipFile\.OpenRead')){
        Add-Finding 'M02-P03-OBS-062' 'ALTA' 'Excel' 'Lectura directa de XML dentro de XLSX' 'El lector abre ZIP y XML.' 'Debe controlar zip bombs, XML externo y tamaños internos.' 'Deshabilitar DTD y limitar entradas, compresión y expansión.'
    }
