...
function Compare-Snapshots {
    ...
    $b = @{}; foreach($x in @($SnapshotBefore)){ $b[$x.Ruta] = $x.SHA256 }
    $a = @{}; foreach($x in @($SnapshotAfter)){ $a[$x.Ruta] = $x.SHA256 }
    @(
...
function Invoke-SafeReadOnlySql {
    param(
        ...
        [string]$Name,[string]$Query,[bool]$Required=$true
    )
    try{
        $dt = Invoke-ReadOnlySql -Connection $Connection -Name $Name -Query $Query
        $SqlChecks.Add([pscustomobject]@{
            Consulta=$Name; Obligatoria=$Required; Estado='OK'
            Filas=$dt.Rows.Count; Error=$null
        })
        Write-Log ("SQL {0}: OK ({1} filas)." -f $Name,$dt.Rows.Count)
        $dt
    }
    catch{
        $message = $_.Exception.Message
        $SqlChecks.Add([pscustomobject]@{
            Consulta=$Name; Obligatoria=$Required; Estado='ERROR'
            Filas=0; Error=$message
        })
