...
$Out = Join-Path $PackageRoot ("RESULTADOS_M02_P03_D01_REV2_" + $Stamp)
...
try {
    ...
    Write-Log 'Inicio diagnostico tecnico M02-P03 D01 REV2 de solo lectura.'

    if (-not (Test-Path -LiteralPath $Root -PathType Container)) {
        throw "No existe la linea base: $Root"
    }
    ...
    if ($missing.Count -gt 0) {
        ...
        throw 'Faltan archivos esenciales de la linea base.'
    }
    ...
    if ($OmitirSql) {
        $SqlState = 'OMITIDO'
        Write-Log 'SQL omitido por parametro.'
    }
    else {
    ...
    try {
        ...
        $conclusion = if ($FinalCode -eq 0) {
            'Diagnostico completado; no habilitar cargas reales hasta evaluar los hallazgos y la compatibilidad de TERCEROS.'
        }
        else {
            'Diagnostico incompleto; revisar ERROR_RAIZ y SQL_ESTADO antes de continuar.'
        }
        $errorText = if ($null -eq $RootError) { '' } else { $RootError }
        $resultLines = New-Object System.Collections.Generic.List[string]
        [void]$resultLines.Add('AGROINPACO ERP - M02-P03 D01 REV2')
        ...
        Write-Log ('Fin del diagnostico. CODIGO_FINAL=' + $FinalCode) $level

        Add-Type -AssemblyName System.IO.Compression.FileSystem
        $zip = Join-Path $PackageRoot ("RESULTADOS_M02_P03_D01_REV2_" + $Stamp + '.zip')
